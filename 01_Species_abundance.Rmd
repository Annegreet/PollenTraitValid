...
# make data tidy
zone_b <- zone_b %>% 
  select(sitename = name, lat, lon, contains("HISTO")) %>% 
...
zone_b_site <- zone_b_site %>% 
  ...
  select(sitename, plot, lat, lon, veg_type) %>% 
...
spec_b <- spec_b_raw %>% 
  ...
  select(sitename = Site, plot, species = Species, abun) %>% 
...
veg_cover_bc <- spec_b %>% 
  ...
  select(sitename, veg_type, species, veg_cov_b, veg_cov_c, spec_abun_b,
         spec_abun_c)
