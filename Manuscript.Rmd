...
Where *n* is the total number of taxa, and $\pi_{j}$ the abundance of each pollen taxon or species at a given site *s*. The mean *CWM~­~^mean^* and the standard deviation *CWM^sd^* of this modelled distribution was subsequently used in the estimation of the correlation between the trait composition of the vegetation and the pollen data.

### Correlation between pollen and vegetation community weighted mean

To test the correlation between the pollen based reconstruction of CWM and the vegetation CWM, we used a multivariate normal distribution to estimate correlation coefficient $\rho$ so that,
...
Correlations were calculated for the different treatments, all zones and three traits: SLA, LA and plant height.

### Community weighted mean changes with elevation

To test if relative trait compositional changes are reflected by the pollen data along an elevation gradient, we fitted a generalized linear model with elevation as the predictor, so that

$$CWM^{mean} \sim Normal(\alpha + \beta Y_{elev}, CWM^{sd})$$

Where $\alpha$ and $\beta$ are the intercept and the slope, respectively, which are supplied by uninformative priors. $Y_{elev}$ is the elevation as the linear predictor.

## Results

### Effect of correction factors

![Estimated correlation between the pollen data and vegetation data. Whisker plots show the median estimated correlation coefficient and the 95% credibility interval.](Figures/Pollen_correction_factors.png)

\* add uncertainty around pollen correction factors

### Pollination mode

![Estimated correlation between the pollen data and vegetation data. Whisker plots show the median estimated correlation coefficient and the 95% credibility interval.](Figures/Pollination_mode.png)

### Plant functional group

![Estimated correlation between the pollen data and vegetation data. Whisker plots show the median estimated correlation coefficient and the 95% credibility interval.](Figures/PFT.png)

### Effect of taxonomic resolution

![Estimated correlation between the pollen data and vegetation data. Whisker plots show the median estimated correlation coefficient and the 95% credibility interval.](Figures/Taxonomic_resolution.png)

### CWM change with elevation

![CWM trait change with elevation for Leaf area, plant height and specific leaf area. Left column shows the CWM values for the vegetation data, right column shows the CWM values for the pollen data. Green lines show the relationships from the linear model, grey area shows the 95% credibility interval. Leaf area and plant height were log transformed before modelling.](Figures/20221016-Swiss_elevation_combined.png)
...
![Scatter plot of vegetation basal area % against pollen abundance %](Figures/Pollen_rep_percent.png)

![Scatter plot of vegetation basal area % against pollen abundance % adjusted with pollen productivity estimates.](Figures/Pollen_rep_adjustedpercent.png)
...
![](Figures/LA_trait_values.png)

![](Figures/PlantHeight_trait_values.png)

![](Figures/SLA_trait_values.png)
